...
NAN_METHOD(dgemm) {
	...
	const double *b = reinterpret_cast<double*>(info[8].As<v8::Float64Array>()->Buffer()->GetContents().Data());
...
NAN_METHOD(sgemm) {
	...
	const int lda = info[7]->Uint32Value();
	const float *b = reinterpret_cast<float*>(info[8].As<v8::Float32Array>()->Buffer()->GetContents().Data());
	const int ldb = info[9]->Uint32Value();
